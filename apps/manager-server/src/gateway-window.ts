...
filterBox.addEventListener('click',function(e){var b=e.target.closest&&e.target.closest('[data-source]');if(!b)return;source=b.getAttribute('data-source');if(source==='all')logs.removeAttribute('data-one-source');else logs.setAttribute('data-one-source','');Array.prototype.forEach.call(filterBox.children,function(c){c.setAttribute('aria-checked',c===b?'true':'false')});filterBtn.setAttribute('aria-pressed','true');filter()});
